...
def generateShp( user, password, db, schema, table, columnOutlet, path ):
    put('../../catchment/Catchment_delineation/getShp.sh', path  + 'getShp.sh') 
    ...
    run('cd {} && echo $PWD && ls && tar -cf basins.tar basin*'.format(path))
    tarPath = path + '/basins.tar'
    get(tarPath,'./shapefiles/basins.tar')
